...
경기장 논리 크기 1600×1000, 상단 y=92, 하단 y=892. Canvas의 +X는 오른쪽, +Y는 아래쪽. 상단 각도는 `[0, π]`, 하단 각도는 `[-π, 0]`. 초기 방향은 상단 `π/2`, 하단 `-π/2`.
...
목표는 대표 데스크톱·태블릿에서 60fps, 2인 5분 플레이 중 지속적인 메모리 증가 없음이다. 원본 PNG는 아트 마스터이며 전송 예산은 메타데이터에 기록한다. 배포 전 필요하면 별도 최적화 사본을 만들어 화질·알파를 비교하고 manifest 경로를 갱신한다. 현재 단계에서 압축 크기 목표를 달성했다고 가정하지 않는다.
