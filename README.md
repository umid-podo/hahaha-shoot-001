...
경기 전 준비 화면에서 자리(P1·P3 지구방위팀, P2·P4 ISB팀)마다 **캐릭터 4명 중 하나와 시작 총기를 자유롭게 고릅니다.** 경기 중에도 자기 패널의 무기 이름 버튼(키보드: P1 `E`, P2 오른쪽 `Shift`, P3 `U`, P4 `Num+`)을 누르면 다음 총기로 바뀝니다. 바꾼 직후 0.4초 동안은 쏠 수 없습니다.

| 총기 | 발사 방식 | 한 발 피해 |
| --- | --- | --- |
| 돌격소총 | 누르고 있으면 0.2초마다 | 5 |
...
가끔 **전투기**가 무작위로 나타나 경기장 가운데를 가로로 지나갑니다. 전투기는 양 팀 총알을 모두 막아 주며(RPG는 그 자리에서 폭발), 화면 안에 있는 동안 **1초마다 위·아래 양방향으로 미사일**을 쏴서 양 팀 모두를 공격합니다. 미사일은 직격 20에 주변 폭발 피해 8(반경 120)이고, 향하는 쪽 레일의 가장 가까운 플레이어를 살짝 유도하며, 엄폐물을 넘어가 플레이어에게 직접 날아갑니다. 빗나가면 목표 레일 선에서 터집니다. 전투기가 지나가는 동안 위치에 따라 좌우로 움직이는 엔진 소리가 나고, 총을 쏠 때마다 총기별 총성이 납니다. 수치는 [`src/game/config.js`](src/game/config.js)의 `WEAPONS`·`JET`·`COVERS`·`COVER`에서 조정합니다. 두 번째 손글씨 메모(무기표·체력·자동 발사·옥상·전투기)를 반영한 업그레이드이며, 이전 버전의 "10점 선취" 규칙은 체력제로 바뀌었습니다.
...
지금 캐릭터 그림은 캐릭터마다 한 장이며 고유 무기가 그려져 있어, 무기를 바꿔도 모습은 그대로입니다. 무기별 그림을 넣으면 든 무기에 따라 모습이 바뀝니다.
...
1. `assets/characters/<캐릭터 id>@<무기 id>.png` 로 저장합니다. 예: `earth-arrow@sniper.png`
   ...
   - 무기 id: `rifle`, `pistol`, `dual`, `rpg`, `sniper`
