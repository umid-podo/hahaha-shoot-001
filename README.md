...
npm test         # node --test, 이동·조준·자동 발사·총기 피해·체력·전투기 로직 검증
```
...
지구방위팀과 ISB팀이 **건물 옥상**의 상·하단 레일에서 좌우(X축)로만 이동하며 대결합니다. 조준 스틱을 드래그해 경기장 안쪽 180도를 조준하면 **누르고 있는 동안 자동으로 발사**됩니다. 모든 플레이어는 **체력 100**으로 시작하며, 캐릭터 아래와 오른쪽 팀 패널에 체력바가 표시됩니다. 체력이 0이 되면 쓰러지고, **상대 팀을 모두 쓰러뜨리면 승리**합니다(같은 순간 양 팀 전멸은 무승부). 아군 공격은 불가능합니다.
...
| 권총 | 0.5초 | 10 |
| 쌍권총 | 0.5초마다 2점사 | 10 |
| RPG | 1.5초 | 직접 명중 20, 폭발 범위(주변 적) 4 |

가끔 **전투기**가 무작위로 나타나 경기장 가운데를 가로로 느리게 지나갑니다. 전투기는 양 팀 총알을 모두 막아 주며, RPG는 전투기에 닿으면 그 자리에서 폭발합니다. 수치는 [`src/game/config.js`](src/game/config.js)의 `WEAPONS`·`JET`에서 조정합니다. 두 번째 손글씨 메모(무기표·체력·자동 발사·옥상·전투기)를 반영한 업그레이드이며, 이전 버전의 "10점 선취" 규칙은 체력제로 바뀌었습니다.
